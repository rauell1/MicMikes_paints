    ...
    messages: [
      {
        role: 'user',
        content: [
          { type: 'text', text: 'Check whether this image is safe and suitable for interior paint recommendation analysis. Return JSON with allowed, reason.' },
...
export async function analyzeRoomImage(imageDataUrl: string) {
  ...
  return nvidiaPost('/chat/completions', {
    ...
    messages: [
      {
        role: 'system',
        content: 'You are an interior paint advisor. Return strict JSON only.'
      },
      {
        role: 'user',
        content: [
          {
            type: 'text',
            text: `Analyze this room photo for paint recommendation.
Return strict JSON:
{
  "room_type": string,
  "lighting": string,
...
export async function generateRecommendationText(analysis: any, catalogContext: string) {
  ...
  return nvidiaPost('/chat/completions', {
    ...
    messages: [
      {
        role: 'system',
        content: 'You are a paint sales assistant. Be concise, practical, and commercial.'
      },
      {
        role: 'user',
        content: `Given this room analysis:\n${JSON.stringify(analysis)}\n\nAnd this paint catalog context:\n${catalogContext}\n\nReturn JSON:
{
  "summary": string,
