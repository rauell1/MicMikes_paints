...
// ---------------------------------------------------------------------------
// Helpers (inlined from mpesa-stk reference repo)
// ---------------------------------------------------------------------------
...
function normalisePhone(raw: string): string {
  ...
  throw new Error(
    `Invalid Kenyan mobile number: "${raw}". ` +
    "Expected 07xxxxxxxx, 010xxxxxxx, 011xxxxxxx or the 254 equivalent."
  );
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { orderId, phone, amountKes } = req.body as {
    orderId: string;
    phone: string;
    amountKes: number;
  };
  ...
  // Amount must be a positive integer (M-Pesa requirement)
  ...
  try {
    normalisedPhone = normalisePhone(String(phone));
  } catch (err) {
    return res.status(400).json({ error: (err as Error).message });
  }

  // Verify order exists in DB and amount matches (server-side trust)
  const sql = neon(process.env.DATABASE_URL!);
  const [order] = await sql`
    SELECT id, total_kes FROM orders WHERE id = ${orderId}
  `;
  if (!order) {
  ...
  if (order.total_kes !== amount) {
    return res.status(400).json({
      error: `Amount mismatch: order total is KES ${order.total_kes}, got KES ${amount}`,
    });
  }
  ...
  const callbackUrl =
    ...
    `https://mic-mikes-paints.vercel.app/api/mpesa/callback`;
  ...
  if (stkData.ResponseCode !== "0") {
    console.error("[mpesa/initiate] STK Push non-zero ResponseCode:", stkData);
  ...
  await sql`
    INSERT INTO mpesa_payments
      (order_id, checkout_request_id, merchant_request_id, phone, amount_kes)
    VALUES
      (${orderId}, ${stkData.CheckoutRequestID!}, ${stkData.MerchantRequestID!},
       ${normalisedPhone}, ${amount})
