...
/* ── Customer order lookup by phone OR email (no auth needed) ── */
async function handleLookup(req: VercelRequest, res: VercelResponse) {
  ...
  if (rawPhone) {
    ...
    orders = await sql`
      SELECT
        o.id, o.created_at, o.status, o.total_kes, o.delivery_kes,
        o.county, o.town,
        (
  ...
  const result = orders.map(o => {
    ...
    return { ...o, reference: ref, items: (o.items as unknown[]) ?? [] };
  });
...
async function handleOrder(req: VercelRequest, res: VercelResponse) {
  ...
  /* ── Link to existing user by email (or phone) ── */
  ...
  try {
    ...
    if (existingUser) {
      ...
      // Auto-create a guest user row so orders are linkable later
      const [newUser] = await sql`
        INSERT INTO users (email, name, phone, role)
        VALUES (${email}, ${name}, ${phone}, 'customer')
        RETURNING id
      `;
      userId = newUser.id as string;
    }
  } catch {
    // Non-fatal — order proceeds without user_id if users table is unavailable
    userId = null;
  }
  ...
  for (const item of items) {
    ...
    verified.push({
      ...
      unitKes:   row.price_kes as number,
    });
  }
  ...
  const [order] = await sql`
    INSERT INTO orders
      (user_id, name, email, phone, county, town, address,
       subtotal_kes, delivery_kes, total_kes, status)
    VALUES
      (${userId}, ${name}, ${email}, ${phone}, ${county}, ${town}, ${address},
  ...
  for (const item of verified) {
    await sql`
      INSERT INTO order_items
        (order_id, product_id, colour_id, size, finish, quantity, unit_kes)
      VALUES
        (${order.id}, ${item.productId}, ${item.colourId},
